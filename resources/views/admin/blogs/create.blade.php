...
<form method="POST" action="{{ route('admin.blogs.store') }}" enctype="multipart/form-data"
    ...
    <div>
        ...
        <select name="product_id" id="product_id"
            ...
            <option value="{{ $p->id }}" {{ old('product_id') == $p->id ? 'selected' : '' }}>{{ $p->title }}</option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <input type="text" name="product_detail_title" id="product_detail_title" value="{{ old('product_detail_title') }}"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent"
    ...
    <div>
        ...
        <textarea name="product_detail_points" id="product_detail_points" rows="5"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent"
            placeholder="One point per line, e.g.&#10;Premium collection of steel tubing.&#10;Crafted using high-quality materials.&#10;Available sizes range from 20mm to 100mm.">{{ old('product_detail_points') }}</textarea>
