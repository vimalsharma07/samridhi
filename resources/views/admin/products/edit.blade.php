...
<div class="mb-8">
    ...
    <p class="mt-1 text-gray-600">Update {{ $product->title }}</p>
</div>
...
<form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data"
    ...
    <div>
        ...
        <textarea name="short_description" id="short_description" rows="2"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent"
            placeholder="Brief summary (used in listings)">{{ old('short_description', $product->short_description) }}</textarea>
    </div>
    ...
    <div>
        ...
        <div class="mb-2">
            <img src="{{ asset('uploads/' . $product->featured_image) }}" alt="" class="h-24 rounded-lg object-cover">
        </div>
        @endif
    ...
    <div>
        ...
        <textarea name="description" id="description" rows="15">{{ old('description', $product->description) }}</textarea>
    ...
    <div>
        ...
        <textarea name="applications_text" id="applications_text" rows="4"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent"
            placeholder="One per line">{{ old('applications_text', $product->applications ? implode("\n", $product->applications) : '') }}</textarea>
    ...
    <div>
        ...
        <textarea name="faqs_text" id="faqs_text" rows="8"
            ...
            placeholder="One FAQ per line: question|answer">{{ old('faqs_text', $product->faqsAsFormText()) }}</textarea>
        <p class="mt-1 text-sm text-gray-500">Accordion on product page. Leave empty to use automatic default FAQs for this product.</p>
    </div>
